...
class DownloadFinam:
    def __init__(
            self, ticker: str, dir_data: str, market: int, daft: int, period: int = 3
            ) -> None:
    ...
    def create_request_finam(self, download_date: str) -> None:
        ...
        start_date: dt = dt.datetime.strptime(download_date, "%Y%m%d").date()  
    ...
    def path_file(self, file_name_date: str) -> Path:
        ...
        return Path(f'{self.dir_data}/{file_name}')  
...
if __name__ == "__main__":
    ...
    start_date_range: dt = dt.date(2025, 1, 18)  # Дата начала закачки
    ...
    data: DownloadFinam = DownloadFinam(ticker, dir_data, market, daft, period)  
    for single_date in date_range:  # Итерируемся по диапазону дат
        ...
        print(download_date)
        # Дата которую будем подставлять в имя файла
        file_name_date: str = single_date.strftime('%Y%m%d')  
