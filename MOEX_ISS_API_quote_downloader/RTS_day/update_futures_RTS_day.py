...
def get_info_future(security: str):
    ...
    """
    # j = requests.get(f'https://iss.moex.com/iss/securities/{security}.json').json()
    j = request_moex(f'https://iss.moex.com/iss/securities/{security}.json').json()
...
def get_future_date_results(tradedate: date, ticker: str):
    ...
    while tradedate <= today_date:
        ...
        if not sqlighter3_RTS_day.tradedate_futures_exists(connection, cursor, tradedate):
            ...
            #     f'date={tradedate.strftime("%Y-%m-%d")}&assetcode={ticker}'
            # ).json()
            ...
            if len(df) != 0:  # Если полученный ответ не нулевой, чтобы исключить выходные дни
                ...
                df[['SHORTNAME', 'LSTTRADE']] = df.apply(
                    lambda x: get_info_future(x['SECID']), axis=1
                )

                df["LSTTRADE"] = pd.to_datetime(df["LSTTRADE"]).dt.date

                # Оставляем строки, где дата последних торгов меньше или равна дате экспирации
...
if __name__ == '__main__':  # Точка входа при запуске этого скрипта
    ...
    # === Удаление последней записи (если есть) ===
