...
def get_future_date_results(
    ...
    print(type(sqlighter3_MIX_day.get_max_date_futures(connection, cursor)))
    print(type(today_date))

    end_date = datetime.strptime(sqlighter3_MIX_day.get_max_date_futures(connection, cursor), "%Y-%m-%d").date()
    ...
    if end_date != today_date:
        ...
        sqlighter3_MIX_day.add_tradedate_future(
            ...
            # today_date
            sqlighter3_MIX_day.get_max_lsttrade(connection, cursor)
            )
...
if __name__ == '__main__':  # Точка входа при запуске этого скрипта
    ...
    if sqlighter3_MIX_day.non_empty_table_futures(connection, cursor):
        ...
        start_date = datetime.strptime(sqlighter3_MIX_day.get_max_date_futures(connection, cursor),
